...
class ReviewPanel {
    ...
    // ── Factory ───────────────────────────────────────────────
    static createOrShow(extensionUri, initialCode, initialLang) {
        ...
        const panel = vscode.window.createWebviewPanel('codeReview', '🔍 Code Review Assistant', column, {
            enableScripts: true,
    ...
    injectCodeFromEditor(code, lang) {
    ...
    async _handleWebViewMessage(message) {
        switch (message.type) {
            ...
            case 'analyze': {
                ...
                const filter = (0, codeFilter_1.filterCode)(code);
                if (!filter.isCode) {
                    this._sendToWebView({
                        type: 'filterError',
